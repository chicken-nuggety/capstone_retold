    ...
    for (int n = 0; n < num_bs; n++){
      for (int i = 0; i<layer; i++){
        for (int j = 0; j < column; j++){
          Books[n][j][i] = new Books(j*(screen_width/column)+(n*screen_width) + xOffset,i*(screen_height/((layer+1)*(layer-1)) + screen_height/(layer+1)) + yOffset,screen_width/column,screen_height/(layer+1));
          Books[n][j][i].create();
        }
      }
    }
